...
async def test_build_basic_context():
    mock_d1 = AsyncMock()
    mock_d1.get_participant.return_value = MagicMock(
        participant_id="P001",
        name="Test User",
        study_group="A",
...
async def test_cache_works():
    mock_d1 = AsyncMock()
    mock_d1.get_participant.return_value = MagicMock(
        participant_id="P001", name="Test", study_group="A",
        ai_type="Relational", day_progress=1, has_onboarded=True, is_disqualified=False,
    )
    mock_d1.get_participant_sessions.return_value = []
    mock_vs = AsyncMock()
    mock_emb = AsyncMock()
    ...
    # Should be cached, so only one D1 call
